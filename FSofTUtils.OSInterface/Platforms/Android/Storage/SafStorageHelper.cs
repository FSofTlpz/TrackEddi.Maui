...
namespace FSofTUtils.OSInterface.Storage {
   ...
   class SafStorageHelper {
      ...
      public DocumentFile? GetExistingDocumentFile(string storagename, string volpath) {
         Android.Net.Uri? rooturi = getTreeDocumentUri(storagename);
         Context? context = AndroidFunction.GetActivityContext(activity);
      ...
      public List<string> ObjectList(string storagename, string volpath, bool dir) {
         ...
         if (doc != null) {
            ...
            foreach (DocumentFile file in doc.ListFiles()) {
               if (dir && file != null && file.IsDirectory && file.Name != null)
                  lst.Add(file.Name);
               if (!dir && file != null && file.IsFile && file.Name != null)
                  lst.Add(file.Name);
            }
         }
         return lst;
      ...
      /// <summary>
      /// liefert eine Liste aus einem <see cref="StorageHelper.StorageItem"/> für den Pfad (Index 0) und den <see cref="StorageHelper.StorageItem"/> für den Inhalt
      ...
      /// <param name="volpath"></param>
      /// <returns></returns>
      public List<StorageHelper.StorageItem> StorageItemList(string storagename, string volpath) {
         List<StorageHelper.StorageItem> lst = new List<StorageHelper.StorageItem>();
         DocumentFile? doc = GetExistingDocumentFile(storagename, volpath);
         if (doc != null) {
            ...
            */
            ...
            return contentResolver == null ?
                     ...
                     mode == "r" ?
                        ...
                        contentResolver.OpenOutputStream(doc.Uri, mode);
         }
         return null;
      }
      ...
      public bool Rename(string storagename, string volpath, string newfilename) {
         ...
         */
         DocumentFile? doc = GetExistingDocumentFile(storagename, volpath);
         bool ok = doc != null ? doc.RenameTo(newfilename) : false;
      ...
      public bool Move(string storagenamesrc, string volpathsrc, string storagenamenewparent, string volpathnewparent) {
         ...
         */
         Android.Net.Uri? srcuri = getDocumentUriUsingTree(storagenamesrc, volpathsrc);
         string? dstpath = Path.GetDirectoryName(volpathsrc);
         Android.Net.Uri? dstpathuri = dstpath != null ? getDocumentUriUsingTree(storagenamenewparent, dstpath) : null;
         Android.Net.Uri? newuri =
            (ContentResolver != null &&
             srcuri != null &&
             dstpathuri != null &&
             dstdoc != null) ?
                  AndroidFunction.DocumentsContractMoveDocument(ContentResolver, srcuri, dstpathuri, dstdoc.Uri) :
                  null;
      ...
      public bool Copy(string storagenamesrc, string volpathsrc, string storagenamenewparent, string volpathnewparent) {
         ...
         DocumentsContract.CopyDocument(provclient, srcdoc.Uri, dstparentdoc.Uri);
               ...
               return outbundle.GetParcelable(DocumentsContract.EXTRA_URI);                        "uri"
         */
         Android.Net.Uri? newuri = ContentResolver != null && srcdoc != null && dstparentdoc != null ?
                                       AndroidFunction.DocumentsContractCopyDocument(ContentResolver,
                                                                                     srcdoc.Uri,
                                                                                     dstparentdoc.Uri) :
                                       null;
...
#pragma warning restore CS8600 // Das NULL-Literal oder ein möglicher NULL-Wert wird in einen Non-Nullable-Typ konvertiert.
      ...
      public
#endif
      Android.Net.Uri? getTreeDocumentUri(string storagename, string volpath = "") {
         if (volpath.Length > 0 &&
             volpath[0] == '/')
         ...
         return AndroidFunction.DocumentsContractBuildTreeDocumentUri(AndroidConstant.AuthorityExternalStorageDocuments, storagename + ":" + volpath);
      }
      ...
      /// <param name="volpath">abs. Pfad im Volume</param>
      /// <returns></returns>
      Android.Net.Uri? getDocumentUriUsingTree(string storagename, string volpath) {
         if (volpath.Length > 0 &&
             volpath[0] == '/')
            volpath = volpath.Substring(1);

         Android.Net.Uri? treeuri = getTreeDocumentUri(storagename);
         // Build URI representing the target Document#COLUMN_DOCUMENT_ID in a document provider. (API level 21 / Lollipop / 5.0)
         return treeuri != null ?
                        AndroidFunction.DocumentsContractBuildDocumentUriUsingTree(treeuri, storagename + ":" + volpath) :
                        null;
      }
      ...
      ContentResolver? ContentResolver => AndroidFunction.GetActivityContentResolver(activity);
      ...
      public bool SetPersistentPermissions(string storagename, int volidx) {
         ...
         try {
            ...
            if (Build.VERSION.SdkInt < BuildVersionCodes.R) {
               Android.Net.Uri? voluri = getTreeDocumentUri(storagename);
               if (voluri == null)
                  return false;
               ...
               if (ContentResolver == null)
                  return false;
               ContentResolver.TakePersistableUriPermission(voluri, flags);
                ...
                */
               foreach (UriPermission perm in ContentResolver.PersistedUriPermissions) {
                  if (voluri.Equals(perm.Uri))
                     ok = true;
               }
            } else {
               ok = AndroidFunction.IsExternalStorageManager; // Ex. die Rechte? (Intent ist nicht nötig)
            }
         } catch (Exception ex) {
      ...
      public void ReleasePersistentPermissions(string storagename, int volidx) {
         ...
         try {
             ...
             */
            ContentResolver?.ReleasePersistableUriPermission(voluri, flags);
         } catch (Exception ex) {
      ...
      /// </summary>
      /// <param name="storagename">Nummer des StorageVolumes; min. 2</param>
      /// <param name="volidx">Index zum Volumenamen</param>
...
#pragma warning disable CS8602 // Dereferenzierung eines möglichen Nullverweises.
                  Tuple<Result, Intent> result = await (Task<Tuple<Result, Intent>>)method.Invoke(activity,
                                                                                                  new object[] {
                                                                                                     intent,
                                                                                                     requestid });
...
#pragma warning restore CS8600 // Das NULL-Literal oder ein möglicher NULL-Wert wird in einen Non-Nullable-Typ konvertiert.
      ...
      Intent? getAccessIntent(string? storagename, int volidx) {
         ...
         else if (Build.VERSION.SdkInt == BuildVersionCodes.Q && storagename != null)
      ...
      Intent? getAccessIntent_PreQ(int volidx) {
         ...
         */
         ...
         if (sm != null) {
            ...
            intent = new Intent(AndroidConstant.AndroidOsStorageActionOpenExternalDirectory);
            intent.PutExtra(AndroidConstant.ExtraStorageVolume, AndroidFunction.GetStorageVolume(sm, volidx));
            intent.PutExtra(AndroidConstant.AndroidOsStorageExtraDirectoryName, null as string);
         }
      ...
      Intent getAccessIntent_Q(string storagename) {
         ...
         Intent intent = new Intent(AndroidConstant.IntentActionOpenDocumentTree);
          ...
          */
         intent.PutExtra(AndroidConstant.DocumentsContractExtraInitialUri, AndroidConstant.ContentExternalStorageTree + storagename + "%3A");
      ...
      Intent? getAccessIntent_R() {
         ...
         if (!AndroidFunction.IsExternalStorageManager) // sonst ex. die Rechte schon
            intent = new Intent(AndroidConstant.ActionManageAllFilesAccessPermission);
