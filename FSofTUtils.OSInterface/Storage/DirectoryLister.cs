...
namespace FSofTUtils.OSInterface.Storage {
   ...
   class DirectoryLister {
      ...
      /// Anzahl der Roots
      /// </summary>
      public int RootCount => Root.Count;
      ...
      /// <summary>
      /// aktueller (zuletzt) mit <see cref="Content(string, bool, bool, Regex, Regex)"/> eingelesener Verzeichnisinhalt (ev. auch leere Liste)
      /// </summary>
      ...
      public List<DirItem> Content(string? path = null,
         ...
         if (!string.IsNullOrEmpty(path) &&
            ...
            DirItem? di = buildItem4Dir(lst, isremovable, isemulated);  // 285ms
            if (di != null)
               ActualContent.Add(di);
         ...
         if (string.IsNullOrEmpty(subdir)) {    // dann keine Änderung
            ...
            if (subdir == "..") {         // nach "oben"
               ...
               if (pathIsRoot(ActualPath)) {
                  ...
                  string? dirname = Path.GetDirectoryName(ActualPath);
                  if (dirname != null) {
                     ActualPath = dirname;
      ...
      /// <summary>
      /// ACHTUNG: rel. zeitintensiv, da für die Infos ein Aufruf von <see cref="StorageHelper.StorageItemList(string,bool)"/> erfolgt
      /// </summary>
