...
namespace FSofTUtils.OSInterface.Control {
   ...
   public partial class SoundPicker : ContentView {
      ...
      public ObservableCollection<SoundDataExt> SoundDataList {
         get;
         private set;
      }
      ...
      /// </summary>
      public SoundDataExt? SelectedSound {
         get {
            if (myCollectionView.SelectedItem != null)
               return (SoundDataExt)myCollectionView.SelectedItem;
            return null;
         }
      }
      ...
      public int SelectedIndex {
         get {
            if (myCollectionView.SelectedItem != null)
               return SoundDataList.IndexOf((SoundDataExt)myCollectionView.SelectedItem);
            return -1;
         }
         set {
      ...
      /// Auswahl erfolgt?
      /// </summary>
      public SoundHelper.SoundData? Result {
         get;
         protected set;
      }
      ...
      ObservableCollection<SoundHelper.SoundData> nativesounddata = [];


      public SoundPicker() {
         InitializeComponent();

         SoundDataList = new ObservableCollection<SoundDataExt>([]);
         BindingContext = this;

         myCollectionView.RemainingItemsThreshold = 15;         // "Nachfüllportionen" für Daten
         myCollectionView.RemainingItemsThresholdReached += MyCollectionView_RemainingItemsThresholdReached;
      }

      public async Task CollectSoundData() {
         BusyChangedEvent?.Invoke(this, true);

         try {
            List<SoundHelper.SoundData> lst = await Sound.SoundHelper.GetNativeSoundData(true, false, true, false, false);
            lst.AddRange(await Sound.SoundHelper.GetNativeSoundData(false, false, true, false, false));
            lst.Sort();
            ...
            nativesounddata = new ObservableCollection<SoundHelper.SoundData>(lst);

            SoundDataList.Clear();
            fillDataToCollectionView();
         } catch {
         ...
         sliderVolume.Value = SoundDataExt.Volume;
      }
      ...
      private void sd_PropertyChanged(object? sender, PropertyChangedEventArgs e) {
         if (e.PropertyName == nameof(SoundDataExt.Active) &&
             sender != null) {
            SoundDataExt sd = (SoundDataExt)sender;
            if (sd.Active) {  // dann diesen auch als SelectedSound
               if (SelectedSound != null && !SelectedSound.Equals(sd)) {
      ...
      public void AppendFile(string fullfilename) {
         addNativeSoundData(new SoundHelper.SoundData(false,
                                             "",
                                             Path.GetFileNameWithoutExtension(fullfilename),
                                             fullfilename));
      }

      void addNativeSoundData(SoundHelper.SoundData nsd) {
         SoundDataExt sd = new SoundDataExt(nsd);
      ...
      private void Item_Tapped(object sender, TappedEventArgs e) {
         // suche 1. Child, das eine Checkbox ist
         if (sender is Border) {
            if (sender is Border border) {
               CheckBox? cb = (CheckBox?)border.Content?.GetVisualTreeDescendants().First(x => x.GetType() == typeof(CheckBox));
               if (cb != null)
      ...
      private void myCollectionView_SelectionChanged(object sender, SelectionChangedEventArgs e) {
         if (e.PreviousSelection != null && e.PreviousSelection.Count == 1) {
            SoundDataExt sd = (SoundDataExt)e.PreviousSelection[0];
            sd.Active = false;
         }

         if (e.CurrentSelection != null && e.CurrentSelection.Count == 1) {
            SoundDataExt sd = (SoundDataExt)e.CurrentSelection[0];
      ...
      private void sliderVolume_ValueChanged(object sender, ValueChangedEventArgs e) {
         ...
         SoundDataExt.Volume = (float)volume;

         labelVolume.Text = "Laustärke " + (volume * 100).ToString("f1") + "%";

         SoundDataExt? sd = SelectedSound;
      ...
      static readonly string[] mimetypes = ["audio/*"];

      /// <summary>
      /// Audio-Dateiauswahl mit dem Xamarin.Essentials.FilePicker
      /// </summary>
      /// <returns></returns>
      async static Task<string> pickAudioFile() {
         string filename = "";
         try {
            var customFileType = new FilePickerFileType(new Dictionary<DevicePlatform, IEnumerable<string>> {
                 { DevicePlatform.Android, mimetypes},
                 /* 
