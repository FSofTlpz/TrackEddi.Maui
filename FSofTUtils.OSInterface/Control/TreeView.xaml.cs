...
namespace FSofTUtils.OSInterface.Control {
   public partial class TreeView : ContentView {
      ...
      public TreeViewNode? SelectedNode {
         ...
         set {
            if (selectedNode == null ||
               ...
               if (oldSelectedNode != null && selectedNode != null) {
      ...
      public static bool RemoveChildNode(StackLayout? xamlChildContainer, TreeViewNode node) {
         node.TreeView?.unregisterTreeViewNodeEvents(node);
         return xamlChildContainer != null ? xamlChildContainer.Children.Remove(node) : false;
      }
      ...
      private void Node_OnBeforeCheckedChanged(object? sender, TreeViewNode.BoolResultEventArgs e) {
         if (sender != null) {
            TreeViewNodeStatusChangedEventArgs args = new TreeViewNodeStatusChangedEventArgs((TreeViewNode)sender);
      ...
      public void ExpandOrCollapseAll(bool expand) {
         foreach (View item in XamlChildContainer.Children) {
            if (item is TreeViewNode)
               ((TreeViewNode)item).ExpandOrCollapseAll(expand);
         }
      }
      ...
      async static void srollToCenter(Element el) {
         ...
         if (parent != null &&
            ...
            await ((ScrollView)parent).ScrollToAsync(el, ScrollToPosition.Center, false);
      }
